...
### plot of DataNeur line i = ith Neuron

N=DataNeur[,1]
for (nneur in 1:Nneur)
{
plot(DataNeur[nneur,2:(N[nneur]+1)],rep(nneur,N[nneur]),xlim=c(0,Tend), ylim=c(0,(Nneur+1)),xlab='',ylab='',pch=16,lwd=0.01)
par(new=TRUE)
}
...
k=8 # number of intervals for reconstruction in the interaction functions
del = 0.005 # length of each interval
...
BoxEst=t(matrix(c(rep(1,2*Tend/10),t(boxes)),ncol=4)) #  repartition of the comportments for each box, each column corresponds to a trial,  first row: ntrial, second: beginning of the box, third:  end of the box, fourth: comportment  number. Here the boxes do not need to be contiguous


######### Lasso procedure
Lasso=BoxLassoHawkes(DataNeur,Nneur,BoxEst,k,del,gamma)


#### plot ...

BLHPlotbis(Lasso,Nneur,k,del,spontComp,interacComp)
