...
\begin{equation}
	\vec{q}\left(\vec{a}\right) = \dfrac{1}{2} \partial_{\vec{a}} \mathcal{H} = G\vec{a} - \vec{b} + \trn{\vec{d}}\partial_{\vec{a}}\abs{\vec{a}}
\end{equation}
...
In such a case, we should get
\begin{equation}
	\forall i, \; d_i \partial\abs{{a}_i} = S_i.
\end{equation}
\begin{itemize}
	\item if $d_i>\abs{S_i}$ then $\tilde{a}_i=0$.
