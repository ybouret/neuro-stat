...
Those matrices  have $N$ columns and $1+NK$ rows.\\
...
The first line are
$$
	\mu_1(0,i) = \mu_2(0,i) = \#\lbrace\lambda_{j,i}\rbrace, \mu_A(0,i) =1.
$$
\item
The following lines are defined for each neurone $l$, using the $K$ values
$$
	\mu_1(1+(Kl)+k,i) = \sum_{\tau\in\lambda_{j,i}} \Phi_{j,l,k,\delta}(\tau)
$$
$$
	\mu_2(1+(Kl)+k,i) = \sum_{\tau\in\lambda_{j,i}} \left(\Phi_{j,l,k,\delta}(\tau)\right)^2
$$
$$
	\mu_A(1+(Kl)+k,i) = \max_{\tau\in\lambda_{j,i}} \left| \Phi_{j,l,k,\delta}(\tau) \right|
$$
\item During aggregation, by kind or by box, we add values or take the maximum value of the target matrix...
\end{itemize}
...
\section{Coefficients Evaluation}
